...
@section('content')
    <div class="card">
        <div class="card-header">
            <strong>Update Transaksi</strong><br>
            <small>{{ $item->trans_id }}</small>
        </div>
        <div class="card-body card-block">
            <form action="{{ route('transaction.update', $item->id) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="{{ old('nama') ? old('name') : $item->name }}" class="form-control @error('name') is-invalid @enderror">
                    @error('name')
